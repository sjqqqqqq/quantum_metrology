#!/bin/bash
#SBATCH --job-name=mle_over_time_v7_loss_notensor
#SBATCH --ntasks=1
#SBATCH --array=0-19
#SBATCH --time=24:00:00
#SBATCH --output=/users/sjqqqqqq/quantum_metrology/slurm_logs/mle_over_time_v7_loss_notensor_%A_%a.out
...
# CONTROL EXPERIMENT for data_mle_over_time_loss_1: identical beam and atom (same gamma_s, loss, shot noise,
# same seeds) but --tensor-scale 0 removes the beta J_z^2 term from the Hamiltonian. This is the
# loss model's "beta = 0" reference: comparing with data_mle_over_time_loss_1 at equal beta isolates what the
# nonlinearity contributes at fixed decoherence, fixed loss and fixed readout noise.
outdir="$repo/data_mle_over_time_loss_1_notensor/task_${SLURM_ARRAY_TASK_ID}"
mkdir -p "$outdir"
