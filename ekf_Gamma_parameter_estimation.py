...
def f(t, x):
    c = np.cos(omega_F * t)
    return np.array([
        -0.5 * (x[3]+M) * x[0] - omega_R * x[1],
         omega_R * x[0] - 0.5 * (x[3]+M) * x[1] + 2.0 * Omega * c * x[2],
        -x[3] * (1.0 + x[2]) - 2.0 * Omega * c * x[1],
         0,
    ])
...
def jac_f(t, x):
    c = np.cos(omega_F * t)
    return np.array([
        [-0.5 * (x[3] + M), -omega_R,        0.0,    -x[1]],
        [ omega_R,     -0.5 * (x[3] + M),    2.0 * Omega * c, x[0]],
        [ 0.0,         -2.0 * Omega * c, -x[3], 0],
        [ 0.0,         0.0,         0.0,         0.0],
    ])
...
x_traj[0] = [1.0/np.sqrt(2), 1.0/np.sqrt(2), 0.0, Gamma]
y = np.zeros(N)
h_clean = np.zeros(N)
...
xhat_post[0] = [0.0, 0.0, 0.0, 8]
P_post[0] = np.diag([1.0, 1.0, 1.0, 100.0])
...
axes.plot(t_plot, x_traj[:, 3], color='k', label=f'{"Gamma"}')
axes.plot(t_plot, xhat_post[:, 3], color='C0', label=f'{"Gamma"}')
axes.legend(loc='upper right'); axes.set_ylabel(f'{"Gamma"}')
