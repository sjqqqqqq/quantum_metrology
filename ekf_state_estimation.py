# %%
# Reproduce Corcione & Tarin (ACC 2023): EKF state estimation for a
# driven, damped two-level system under continuous measurement.
# State estimation only (paper Sec. IV, Figs. 2-6).
...
Omega = 3.0 * Gamma    # Rabi freq
omega_F = 4.0 * Gamma  # drive freq
...
def f(t, x):
    c = np.cos(omega_F * t)
    return np.array([
        -0.5 * (Gamma+M) * x[0] - omega_R * x[1],
         omega_R * x[0] - 0.5 * (Gamma+M) * x[1] + 2.0 * Omega * c * x[2],
...
def jac_f(t):
    c = np.cos(omega_F * t)
    return np.array([
        [-0.5 * (Gamma+M), -omega_R,        0.0],
        [ omega_R,     -0.5 * (Gamma+M),    2.0 * Omega * c],
