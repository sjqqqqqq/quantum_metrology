...
def f(t, x):
    c = np.cos(omega_F * t)
    return np.array([
        -0.5 * (Gamma+M) * x[0] - x[3] * x[1],
         x[3] * x[0] - 0.5 * (Gamma+M) * x[1] + 2.0 * Omega * c * x[2],
        -Gamma * (1.0 + x[2]) - 2.0 * Omega * c * x[1],
         0,
    ])

def g(x):
    return sqrt_etaM * np.array([-x[0] * x[2], -x[1] * x[2], 1.0 - x[2] ** 2, 0.0])

def jac_f(t, x):
    c = np.cos(omega_F * t)
    return np.array([
        [-0.5 * (Gamma + M), -x[3],        0.0,    -x[1]],
        [ x[3],     -0.5 * (Gamma + M),    2.0 * Omega * c, x[0]],
        [ 0.0,         -2.0 * Omega * c, -Gamma, 0],
        [ 0.0,         0.0,         0.0,         0.0],
    ])
...
x_traj[0] = [1.0/np.sqrt(2), 1.0/np.sqrt(2), 0.0, omega_R]
y = np.zeros(N)
h_clean = np.zeros(N)
...
I4 = np.eye(4)
R = dt                            # Var(dW/dt) = 1/dt
C = np.array([[0.0, 0.0, sqrt_etaM, 0.0]])
#Q = np.diag([1.0, 1.0, 1.0/10.0, 0.0, 0.0]) * dt
xhat_prior = np.zeros((N + 1, 4))
xhat_post = np.zeros((N + 1, 4))
P_post = np.zeros((N + 1, 4, 4))

xhat_post[0] = [0.0, 0.0, 0.0, 45]
P_post[0] = np.diag([1.0, 1.0, 1.0, 400.0])
...
for k in range(N):
    ...
    A = I4 + 0.5 * dt * (jac_f(tk, xp) + jac_f(tk + dt, x_mid))
    ...
    K = (P_pri @ C.T + G_pri*dt) / S[0, 0]      # 5x1
    #K = (P_pri @ C.T ) / S[0, 0]
    innov = y[k] - sqrt_etaM * x_pri[2]*dt
...
axes.plot(t_plot, x_traj[:, 3], color='k', label=f'{"omega_R"}')
axes.plot(t_plot, xhat_post[:, 3], color='C0', label=f'{"omega_R"}')
axes.legend(loc='upper right'); axes.set_ylabel(f'{"omega_R"}')
axes.set_xlabel('time (1/Gamma)')
fig.suptitle('Truth vs EKF estimate')
