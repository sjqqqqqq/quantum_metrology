# %%
"""Open-system (v5) variant of multiparameter_trotter_v4_least_squares_over_time.py.

The v4 model is closed: H = w.J + Omega*(cos(phi) J_x + sin(phi) J_y) + beta/(2J) J_z^2. Here the
beta J_z^2 term is understood as the tensor light shift of an off-resonant, linearly polarized
beam on a single atomic spin (Cs F=3 for J=3), and the same beam's photon scattering is added
as the ONE decoherence channel of the model. In the far-detuned limit the no-spin-flip
(Rayleigh) part of the scattering is scalar and does not decohere; the electron-spin-flip
(Raman) part, projected onto the F manifold, is proportional to J_+/J_- (Wigner-Eckart), so
the within-manifold dissipator is the transverse spin-flip channel

    D[rho] = gamma * sum_{s=+,-} ( J_s rho J_s^dag - 1/2 {J_s^dag J_s, rho} ),

with gamma tied to beta by a fixed atomic figure of merit eta = beta/gamma (both beta and the
scattering rate scale as intensity/detuning^2, so their ratio is a property of the atom, of
order Delta_HF'/Gamma). Exactly (for any J) this channel relaxes <J_x>, <J_y> at rate gamma
and <J_z> at rate 2*gamma. Loss to the other hyperfine manifold and any decoherence from the
readout beam are deliberately NOT modeled (separate readout beam, trace-preserving channel).

With --eta omitted the script reduces exactly to v4 (closed system, same RNG stream, same
outputs); with --eta the per-beta rate gamma = beta/eta is applied as one extra
w-independent map per Trotter step, so the least-squares fit and Fisher-information
derivatives are otherwise unchanged. gamma_array.npy records the rate used for each beta.
"""
...
parser.add_argument("--eta", type=float, default=None, help="light-shift figure of merit eta = beta/gamma: each beta runs with spin-flip decoherence rate gamma = beta/eta (transverse <J_x>,<J_y> relaxation rate; <J_z> relaxes at 2*gamma). Omit for the closed system (identical to v4)")
args = parser.parse_args()
assert args.eta is None or args.eta > 0, "--eta must be positive (omit it for the closed system)"
...
J_plus = J_x + 1j * J_y
J_minus = J_x - 1j * J_y
gamma_array = np.zeros(len(beta_array)) if args.eta is None else beta_array / args.eta #per-beta spin-flip rate
...
def w_estimate_over_time(gaussian_noise, expectation_true, w_starts, time_cutoffs, U_c, U_c_dagger, A):
...
def compute_fisher_matrix(del_expect_x, del_expect_y, del_expect_z, t_cutoff):
    ...
    control realization, using only measurements up to t_cutoff (inclusive)."""
    n_sub = int(n_meas_cum[t_cutoff])
...
def run_realization(w_true, beta_val, gamma_val, phi, gaussian_noise, w_starts, time_cutoffs):
    ...
    gamma_val = beta_val/eta is this beta's decoherence rate (0 for the closed system); its
    dissipative step map is built once here (a dim^2 x dim^2 expm) and threaded through the
    trajectory, the fit residuals and the Fisher derivatives.
    """
    ...
    H_c_unique = H_c(phi_unique, beta_val, omega, J)
    ...
    fit_results = w_estimate_over_time(gaussian_noise, expectation_true, w_starts, time_cutoffs, U_c, U_c_dagger, A)
...
def run_all_betas(w_true):
    ...
    for _ in range(iter // batch_size):
        ...
        noise_batch = [rng.normal(0, sn_sd, measurement_steps) for _ in range(n_tasks)]
        w_starts_batch = rng.uniform(-w_bound, w_bound, size=(n_tasks, n_time, n_extra_starts, 3))
        batch_results = Parallel(n_jobs=args.n_jobs)(
            delayed(run_realization)(w_true=w_true, beta_val=beta_array[b], gamma_val=gamma_array[b], phi=phi,
                                      gaussian_noise=n, w_starts=ws, time_cutoffs=time_cutoffs_array)
        ...
        #estimate). This is independent of phi and beta, so it's read straight off noise_batch.
...
parameters = (
    ...
    f"Decoherence: eta = {args.eta} (None = closed system, identical to v4), gamma_array = beta_array/eta = {gamma_array} "
    f"(spin-flip channel L = sqrt(gamma) J_+/J_-, transverse relaxation rate gamma, longitudinal 2*gamma; "
    f"trace preserving, no loss channel, no readout-beam decoherence), "
...
np.save(os.path.join(args.outdir, "gamma_array.npy"), gamma_array) #per-beta decoherence rate (all zero for the closed system)
