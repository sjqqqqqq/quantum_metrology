...
parser.add_argument("--iter", type=int, default=500, help="fits collected per cutoff")
...
#Control Hamiltonian: a fixed cyclic pulse sequence, not a per-realization random draw (unlike
#H_c in multiparameter_trotter_v4_least_squares_over_time.py, a randomized rotation axis in
#the x-y plane plus a beta/one-axis-twisting term). Each segment is a pure rotation about a
#single axis (J_z, then J_x, then J_y) held for exactly a quarter turn -- (pi/2)/omega -- so
#that, absent w_true, the SCS is carried exactly x -> y -> z -> x -> ..., repeating for the
#whole run. w_true (applied every step via the Trotter interleaving in compute_expectations/
#residuals below) is what makes the trajectory deviate from that clean cycle; that deviation
#is the signal the least-squares fit uses to recover w. Since the control no longer depends on
#any random draw, U_c/U_c_dagger are built once here rather than per realization.
def build_cyclic_control(omega, delta_t, t_steps):
    cycle_generators = [J_z, J_x, J_y]  # x->y about z, y->z about x, z->x about y
    steps_per_segment = int(round((np.pi/2) / (omega*delta_t)))
    assert steps_per_segment > 0, "omega too large for delta_t: a quarter-turn segment collapses to zero steps"
    U_c_segments = np.array([expm(-1j*delta_t*omega*gen) for gen in cycle_generators])
    n_cycle_steps = 3*steps_per_segment
    n_full_cycles = int(np.ceil(t_steps / n_cycle_steps))
    U_c = np.tile(np.repeat(U_c_segments, steps_per_segment, axis=0), (n_full_cycles, 1, 1))[:t_steps]
    U_c_dagger = np.ascontiguousarray(U_c.conj().transpose(0, 2, 1))
    return U_c, U_c_dagger, steps_per_segment

U_c, U_c_dagger, steps_per_segment = build_cyclic_control(omega, delta_t, t_steps)
...
def compute_expectations(w, U_c, U_c_dagger):
    """U_c and U_c_dagger are explicit arguments (not module globals): joblib memory-maps
    large array arguments to workers rather than cloudpickling globals into every task
    payload (see the residuals docstring for the same rationale).
    """
...
def compute_fisher_matrix(del_expect_x, del_expect_y, del_expect_z, t_cutoff):
    """Classical Fisher information for w from the noiseless-trajectory derivatives of the
    (fixed) control, using only measurements up to t_cutoff (inclusive)."""
    n_sub = int(n_meas_cum[t_cutoff])
...
def run_realization(w_true, gaussian_noise, w_starts, time_cutoffs, U_c, U_c_dagger):
    """One full noise realization, run entirely inside a joblib worker.

    U_c and U_c_dagger are explicit arguments (not module globals): joblib memory-maps large
    array arguments to workers rather than cloudpickling globals into every task payload (see
    the residuals docstring for the same rationale). Unlike a per-realization random control,
    the cyclic pulse sequence is fixed for the whole run -- built once by build_cyclic_control
    above -- so every realization (and every trial) shares the exact same U_c/U_c_dagger; only
    the injected noise and the random retry starts differ realization to realization.
    """
    ...
    return fit_results, fisher_matrices

def run_all(w_true):
    """Run `iter` independent noise realizations for one true parameter vector w_true, fitting
    w at every time cutoff, and return a dict of per-time-cutoff statistics.

    There is no beta/control sweep here (unlike multiparameter_trotter_v4_least_squares_over_
    time.py): the control is the single fixed cyclic pulse sequence built once above, so every
    realization uses the same U_c/U_c_dagger and only the noise (and retry starts) vary.
    """
    covar_array = np.zeros((n_time, 3, 3))
    mean_array = np.zeros((n_time, 3))
    std_array = np.zeros((n_time, 3))
    fisher_array = np.zeros((n_time, 3, 3))
    bias = np.zeros((n_time, 3))
    CRB_diff_eig = np.zeros((n_time, 3), dtype=complex)
    max_cost_estimate = np.zeros(n_time)
    min_cost_estimate = np.full(n_time, np.inf)
    cost_true = np.zeros(n_time)
    cost_true_sum = np.zeros(n_time)
    cost_true_count = 0
    cost_est_kept = np.zeros((n_time, iter_save)) #Costs of the kept fits
    accept_fraction = np.zeros(n_time) #QA metric: fraction of all collected fits within the chi-square band
    fit_success = np.zeros((n_time, iter), dtype=bool) #least_squares success flag per fit
    w_est = np.zeros((n_time, iter_save, 3))

    #Collect `iter` fits at every time cutoff. Each repetition draws one noise realization,
    #reused (sliced) across all cutoffs, so w_est_over_time[j] holds the estimates for cutoff
    #j. Every result is kept (with its success flag) so realization k stays aligned across
    #cutoffs. The noise and the random retry starts are pre-drawn in the main process so
    #workers never touch the global rng and the run is reproducible for a given seed.
    w_est_over_time = [[] for _ in range(n_time)]
    fisher_sum = np.zeros((n_time, 3, 3))
    ...
    for _ in range(iter // batch_size):
        ...
        batch_results = Parallel(n_jobs=args.n_jobs)(
            delayed(run_realization)(w_true=w_true, gaussian_noise=n, w_starts=ws,
                                      time_cutoffs=time_cutoffs_array, U_c=U_c, U_c_dagger=U_c_dagger)
        ...
        for j, t_cutoff in enumerate(time_cutoffs_array):
            cost_true_sum[j] += np.sum(noise_cumsq[:, int(n_meas_cum[t_cutoff]) - 1])
        cost_true_count += batch_size

        for res_list, fisher_list in batch_results:
            for j, res in enumerate(res_list):
                w_est_over_time[j].append((res.x, res.cost, res.success))
            for j, fm in enumerate(fisher_list):
                fisher_sum[j] += fm

    cost_true = cost_true_sum / cost_true_count

    for j, t_cutoff in enumerate(time_cutoffs_array):
        all_costs = np.array([cost for w, cost, success in w_est_over_time[j]])
        fit_success[j] = np.array([success for w, cost, success in w_est_over_time[j]])
        if iter_save < iter:
            #Optional cost-based truncation (off by default: it biases the statistics)
            order = np.argsort(all_costs)[:iter_save]
        else:
            order = np.arange(iter) #Keep realization order so w_est[j, k] is realization k at every cutoff
        w_est[j] = np.array([w_est_over_time[j][i][0] for i in order])
        cost_est_kept[j] = all_costs[order]
        max_cost_estimate[j] = all_costs[order].max()
        min_cost_estimate[j] = all_costs.min()
        ...
        accept_fraction[j] = np.mean(all_costs <= cost_accept_threshold(t_cutoff))

        #Calculate covariance and mean of w estimates at this time cutoff
        covariance = np.cov(w_est[j], rowvar=False)
        covar_array[j] = covariance
        mean_array[j] = np.mean(w_est[j], axis=0)
        bias[j] = mean_array[j] - np.array(w_true)
        std_array[j] = np.diag(covariance)

        #Fisher information, averaged over the `iter` independent noise realizations
        #collected at this cutoff (the control -- and thus the noiseless trajectory
        #derivatives -- is identical across realizations, so this mainly averages out
        #numerical noise; kept for parity with the randomized-control script).
        fisher_matrix = fisher_sum[j] / iter
        fisher_array[j] = fisher_matrix

        CRB_diff_eig[j] = np.linalg.eigvals(covariance - np.linalg.inv(fisher_matrix))
    ...
    infidelity_values = np.array([
        1 - fidelity(fisher_inv_normalized[j], cov_array_normalized[j]) for j in range(n_time)
    ])
...
# %%
#Run the estimation once per true-parameter draw (trial), then average the summary statistics
#over trials. n_trials = 1 reproduces the original single-w_true behavior.
n_trials = args.n_trials

#Raw per-realization fields (tied to one trial's own w_true, not meaningfully averaged across
#different w_true draws) vs. summary-statistic fields (relative/normalized quantities where
#averaging across trials is exactly the "average over true parameter choices" the user asked
#for). Maps each key to the base output filename used below.
FILE_NAME_MAP = {
    ...
    "cost_true": "cost_true", "cost_est_kept": "cost_est",
    "accept_fraction": "accept_fraction",
...
#Leading axis on every field below is now the trial axis: shape (n_trials, n_time, ...)
...
parameters = (
    f"Parameters: J = {J}, omega = {omega}, w_stdev = {w_stdev:.3f}, sn_sd = {sn_sd:.3f}, "
    f"delta_t = {delta_t}, t_steps = {t_steps}, iter = {iter}, iter_save = {iter_save}, "
    ...
    f"Control: fixed cyclic pulse sequence, not the randomized-phase control of "
    f"multiparameter_trotter_v4_least_squares_over_time.py (no beta/one-axis-twisting term either). "
    f"Each segment is a pure rotation of magnitude omega = {omega} about J_z, then J_x, then J_y in "
    f"turn, held for steps_per_segment = {steps_per_segment} steps (a quarter turn, (pi/2)/omega), and "
    f"repeated for the whole run, so absent w_true the SCS is carried exactly x -> y -> z -> x -> ...; "
    f"w_true (applied every step via the Trotter interleaving in compute_expectations/residuals) is what "
    f"makes the trajectory deviate from that clean cycle -- that deviation is the fit signal. The same "
    f"U_c/U_c_dagger is used for every realization and every trial (see build_cyclic_control). "
    f"n_trials = {n_trials} true-parameter draws (see w_true.npy, shape (n_trials, 3))."
)
...
#Full per-trial data, shape (n_trials, n_time, ...) -- n_trials=1 keeps the array shape from a
#single extra leading axis of size 1 relative to the pre-multi-trial version.
for key, val in per_trial_results.items():
    np.save(os.path.join(args.outdir, f"{FILE_NAME_MAP[key]}.npy"), val)

#Trial-averaged summary statistics, shape (n_time, ...) -- this is the "average the gathered
#statistics over all the different choices of the true parameter" output.
